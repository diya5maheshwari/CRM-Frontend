...
export default function Leads({ type }) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await fetch(`https://crm-2umd.onrender.com/api/crm/delete/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
          ...
          <tbody>
            {currentLeads.map((l, index) => (
              <tr key={l.leadId} className="border-t">
                ...
                <td className="p-3 text-blue-600">{l.leadTitle}</td>
                ...
                <td className="p-3">{formatDate(l.nextFollowUpDate)}</td>
